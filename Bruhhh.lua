...
make("UIGridLayout", {
    CellSize            = UDim2.new(0.333, -3, 0, 28),
    CellPaddingOffset   = UDim2.new(0, 3, 0, 3),
    SortOrder           = Enum.SortOrder.LayoutOrder,
...
local function buildRemoteList()
    ...
    for order, remote in ipairs(REMOTES) do
        ...
        local parentBtn = make("TextButton", {
            Text             = (hasDupes and "\u{25B6} " or "\u{25B6} ") .. getShortName(remote.path),
            Font             = Enum.Font.Code,
            TextSize         = 11,
            TextColor3       = C.White,
            BackgroundColor3 = C.Toolbar,
